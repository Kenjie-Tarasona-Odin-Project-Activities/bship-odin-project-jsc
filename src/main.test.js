...
describe("Game Board Position Check : Valid Positions Ship size 4", () => {
  ...
  it("Inbounds", () => {
    expect(board.checkForOutOfBoundsPosition(0, 3)).toBe(true);
  });

  it("Ship Fits: Horizontal Orientation", () => {
    expect(board.checkForShipUnfitInPosition(testShip, 3, 2)).toBe(true);
  });

  it("Ship Too Large: Vertical Orientation", () => {
    testShip.rotate();
    expect(board.checkForShipUnfitInPosition(testShip, 2, 2)).toBe(true);
  });
});
